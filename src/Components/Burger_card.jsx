const Burger_card = (props) => {
  return (
    <>
      <div className="p-2 lg:w-[276px] lg:h-[421px] w-[145px] h-[243px] bg-white lg:p-3 rounded-[12px] lg:rounded-[18px]">
        <img
          src={props.img}
        ...
        <p className="text-gray-txt text-xs font-semibold mt-4 mb-[7px] lg:text-base lg:mt-[29px] lg:mb-2">
          {props.weight}
        </p>
        <button
          className="w-full bg-gray-medium h-[30px] lg:h-10 rounded-lg text-xs font-normal lg:text-base hover:bg-gray-200"
          onClick={() => {
            props.handleAddBurger(props.index);
            // props.onClick(props.index);
            props.setClickedBurger(props.index);
          }}
        >
          Add
        </button>
